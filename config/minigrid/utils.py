...
def copy_minigrid(env, env_name, agent_view_size=7):
    """Copy a Minigrid environment"""
    env_copy = gym.make(
        env_name,
        agent_view_size=agent_view_size,
    )
    env_copy.reset()
    env_copy.unwrapped.grid = env.unwrapped.grid
    env_copy.unwrapped.agent_pos = env.unwrapped.agent_pos
    env_copy.unwrapped.agent_dir = env.unwrapped.agent_dir
    return env_copy


def prepare_observation_lst(observation_lst):
    """Prepare the observations to satisfy the input fomat of torch
    [B, S, W, H, C] -> [B, S x C, H, W]
    """
    # B, S, W, H, C
